Global SelectionMethods := Array("Advanced", "CtrlA", "ArrowSpam")
...
Menu_Toggle(A_TrayMenu, "Ignore Actions", "Misc", "IgnoreActions", "1&")
A_TrayMenu.Insert("1&")

MacrosSubmenu := Menu()
Menu_Macros(MacrosSubmenu, MacroNames, "Misc", "ActiveMacros")
A_TrayMenu.Insert("1&", "Macro Preset", MacrosSubmenu)
...
Menu_Toggle(A_TrayMenu, "Press enter", "Misc", "HotkeyEnter", "1&")
...
Menu_Macros(MenuObj, Names, Section, Key, Pos := 0) {
	for i,Name in Names {
		if Pos != 0
			MenuObj.Insert(Pos, Name, Fn)
		else
	...
	Fn(ItemName, ItemPos, MyMenu) {
		if Conf[Section][Key] != ItemName {
			...
			OldMacros := Macros[Conf[Section][Key]]

			Conf[Section][Key] := ItemName
			IniWrite(Conf[Section][Key], Path_Conf, Section, Key)

			UpdateMacros(GetActiveMacros(), OldMacros)
		}
	}
}
